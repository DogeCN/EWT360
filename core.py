...
class Ewt_upload_progress(functions.EWT_Functions):
    ...
    def run(self,cls):
        ...
        residue_time_ms = round((lesson_duration*1000*0.8 - lesson_duration*1000*cls['ratio']+5000)/self.speed)
        secret,x_bfe_session_id,begin_ts = self.get_bfe_secret_v3()
        begin_time = int(begin_ts)
        time_finished_ms = 0
        # threading.Thread(target=self.uploadprogress_v3, args=(secret,x_bfe_session_id,begin_time,report_time,lesson_id,course_id,i,action,duration)).start()
        time.sleep(0.5)
        threading.Thread(target=self.uploadprogress_v3, args=(secret,x_bfe_session_id,begin_time,functions.now_timestamp(),lesson_id,course_id,1,1,0)).start()
        i = 2
        while time_finished_ms<residue_time_ms:
            if residue_time_ms-time_finished_ms >= 60000:
                time.sleep(60.1)
                threading.Thread(target=self.uploadprogress_v3, args=(secret,x_bfe_session_id,begin_time,functions.now_timestamp(),lesson_id,course_id,i,2,60000)).start()
                threading.Thread(target=self.update_lesson_ratio, args=(cls,)).start()
                #try: self.update_lesson_ratio(cls)
                #except: pass
                time_finished_ms += 60000
                i += 1
                #self.update_tableview_signal.emit()
            elif 0 < residue_time_ms-time_finished_ms < 60000:
                time.sleep((residue_time_ms-time_finished_ms)/1000 + 0.1)
                duration = residue_time_ms-time_finished_ms
                threading.Thread(target=self.uploadprogress_v3, args=(secret,x_bfe_session_id,begin_time,functions.now_timestamp(),lesson_id,course_id,i,2,duration)).start()
                time_finished_ms += duration
                i += 1
            else:
                raise Exception
        threading.Thread(target=self.uploadprogress_v3, args=(secret,x_bfe_session_id,begin_time,functions.now_timestamp(),lesson_id,course_id,i,3,0)).start()
    ...
    def start_run(self):
        try:
            ...
            for i in range(0,len(self.lessons)):
                item = self.lessons[i]
                if item['ratio']<1:
                    time_remain += (0.8-item['ratio'])*item['duration']
            time_remain = round(time_remain/self.speed + len(self.lessons)*2)
